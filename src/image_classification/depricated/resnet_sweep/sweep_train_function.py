...
RESULTS_DIR = "/scratch/outputs"


def train(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, config, start_epoch, last_layer_trained ,enable_logging=False):
    """
    ...
    def save_model(epochs, model, optimizer, criterion, last_layer_trained):
        """
        Function to save the trained model to disk.
        """
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        model_filename = f"ex14_final_model_{timestamp}.pth"
        out_path = os.path.join(RESULTS_DIR, model_filename)
        torch.save(
            {
                "epoch": epochs,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if epoch % 1 == 0:
            print(f"Saving current epoch: {epoch}")
            timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
            chkp_path = os.path.join(
                RESULTS_DIR, f"ex14_checkpoint_{epoch}_{timestamp}.pth"
            )
            torch.save(
                {
                    "epoch": epoch,
        ...
        save_model(num_epochs, model, optimizer, criterion, last_layer_trained)
