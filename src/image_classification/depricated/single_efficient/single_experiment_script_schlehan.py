...
from pipeline.image_classification_pipeline.template.heads.rgz_small_head import RGZ_SmallHead
from pipeline.image_classification_pipeline.template.train_function import train as train_func
from exp_pipeline import Pipeline
...
# datapaths
dest_path=f"/raid/persistent_scratch/schlehan/jobs/efficient_net_single"
...
# load base config
base_config = OmegaConf.load("../single_efficient/omega_conf/configuration.yaml")

# load override config, to merge later
override_config = OmegaConf.load("../single_efficient/omega_conf/override.yaml")
...
model = RGZ_SmallHead(model, in_features=1280, num_classes=6, dropout_rate=0.5)
#b3 1536
...
transform = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.485, 0.456, 0.406], 
                                     std=[0.229, 0.224, 0.225])  # Drei Kanäle normalisieren
])
