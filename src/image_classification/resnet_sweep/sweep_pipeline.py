...
class Pipeline:
    ...
    def init_wandb(self):
        ...
        #     # name=self.config.run_name, # run name could be defined here
    ...
    def setup_dataloaders(self, dataset, train_size, val_size):
        ...
        train_loader = DataLoader(train_set, batch_size=self.config.batch_size, shuffle=True, collate_fn=self.collate_fn) # todo. alter this later to shuffle = true again
        val_loader = DataLoader(val_set, batch_size=self.config.batch_size, shuffle=False, collate_fn=self.collate_fn)
...
"""
        ...
        if self.config.run_on_cluster and self.config.enable_logging:
            ...
            tarball_save_path = f"/cluster/projects/ml4gleam/{self.config.name}_{self.config.name}"
            os.makedirs(tarball_save_path, exist_ok=True)
            tarball_name = f"{self.config.name}_{self.config.name}_train_{actual_uuid}.tar.gz"
