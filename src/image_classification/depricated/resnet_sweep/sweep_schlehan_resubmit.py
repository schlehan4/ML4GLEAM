...
from sweep_pipeline import Pipeline
from sweep_train_function import train as train_func
...
def objective(config):
    ...
    last_layer_is_trained=0
    start_epoch=0
    checkpoint_files = sorted(glob.glob(f"{RESULTS_DIR}/ex14_checkpoint_*.pth"))
    ...
    if latest_checkpoint:
        try:
            ...
            last_layer_is_trained = checkpoint.get("last_layer")
    ...
    exp.setup_model(model=model,
                    ...
                    criterion=criterion,
                    scheduler=scheduler,
                    last_layer_is_trained=last_layer_is_trained,
...
def main():
    wandb.init(project="my-new-sweep")
...
# 3: Start the sweep
sweep_id = wandb.sweep(sweep=sweep_configuration, project="my-new-sweep", entity="PABA-ML4GLEAM")
